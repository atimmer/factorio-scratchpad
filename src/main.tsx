...
function App() {
  ...
  return <main>
    ...
    <section className="sheet" aria-label="Calculation scratchpad">
      ...
      <div className="column results"><div className="column-heading"><h2>02 <strong>Live results</strong></h2><span className="live">{count} CALCULATED</span></div><div className="output" aria-label="Calculated notes">{rows.map((row, index) => <div className={`result-line ${row.value !== undefined ? 'calculated' : ''} ${row.error ? 'invalid' : ''}`} key={index}><span className="source">{row.source || '\u00a0'}</span>{row.value !== undefined ? <strong className="answer">{format(row.value)}</strong> : null}{row.error ? <span className="error">{row.error}</span> : null}</div>)}</div></div>
    </section>
